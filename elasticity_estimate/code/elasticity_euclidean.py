...
#X = sm.add_constant(X)
reg = lm.PanelOLS.from_formula('ln_earnings ~ gamma0 + gamma1 + gamma2 + gamma3 + Weeks + EntityEffects + TimeEffects', data=X, drop_absorbed=True)
...
with open('output/euclid_regression.tex', 'w') as f:
    f.write(latex)
...
fig.tight_layout()
plt.savefig('output/cross_elasticity_euclid.png')
